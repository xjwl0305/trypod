...
exports.deviceGetBranch = async (uid, branch_name) => {
    const data = await sequelize.query('select A.id, B.device_number, i.name, B.order_weight, l.branch_detailed_address, B.created_at from (select earlivery_device_id, max(device_raw_data.created_at) as max_date from device_raw_data group by earlivery_device_id) as t2, device_raw_data as A\n' +
        ...
        'where u.id = :uid and branch_name = :branch_name and t2.earlivery_device_id = A.earlivery_device_id and t2.max_date = A.created_at order by B.device_number',
...
exports.deviceGetLayer = async (uid, branch_name, layer_name) => {
    const data = await sequelize.query('select A.id, B.device_number, i.name, B.order_weight, l.branch_detailed_address, B.created_at from (select earlivery_device_id, max(device_raw_data.created_at) as max_date from device_raw_data group by earlivery_device_id) as t2, device_raw_data as A\n' +
        'left join earlivery_device B on A.earlivery_device_id = B.id left join item i on B.item_id = i.id left join location l on B.location_id = l.id left join user u on l.user_id = u.id\n' +
        'where u.id = :uid and branch_name = :branch_name and layer_name = :layer_name and t2.earlivery_device_id = A.earlivery_device_id and t2.max_date = A.created_at order by B.device_number',
...
exports.deviceGetHouse = async (uid, branch_name, layer_name, warehouse_name) => {
    const data = await sequelize.query('select A.id, B.device_number, i.name, B.order_weight, l.branch_detailed_address, B.created_at from (select earlivery_device_id, max(device_raw_data.created_at) as max_date from device_raw_data group by earlivery_device_id) as t2, device_raw_data as A\n' +
        'left join earlivery_device B on A.earlivery_device_id = B.id left join item i on B.item_id = i.id left join location l on B.location_id = l.id left join user u on l.user_id = u.id\n' +
        'where u.id = :uid and branch_name = :branch_name and layer_name = :layer_name and warehouse_name = :warehouse_name and t2.earlivery_device_id = A.earlivery_device_id and t2.max_date = A.created_at order by B.device_number',
...
exports.deviceUpdateData = async (uid, device_num) => {
    ...
    const data5 = await sequelize.query('select device_number, item_id, location_id, container_id from earlivery_device where device_number = :device_num',
        {replacements: { uid: uid , device_num: device_num}, type: QueryTypes.SELECT});
    const pre_data = {"pre_data": data5};
    return Object.assign(item_list, container_name, container_list, where, pre_data);
}
...
exports.branchList = async (uid) => {
    return await sequelize.query('select distinct branch_name, branch_address, manager_name, manager_phone, manager_email from location left join user u on location.user_id = u.id where u.id = :uid and layer_name is null',
        {replacements: {uid: uid}, type: QueryTypes.SELECT});
}

exports.layerList = async (uid, branch_name) => {
    return await sequelize.query('select distinct layer_name, manager_name, manager_email, manager_phone from location as l left join warehouse_raw_data w on l.id = w.location_id left join user u on l.user_id = u.id where u.id = :uid and branch_name = :branch_name and warehouse_name is null and layer_name is not null',
        {replacements: {uid: uid, branch_name: branch_name}, type: QueryTypes.SELECT});
}

exports.warehouseList = async (uid, branch_name, layer_name) => {
    return await sequelize.query('select distinct warehouse_name, temperature, humidity, max_temp, min_temp, max_hum, min_hum, manager_name, manager_email, manager_phone, w.created_at from (select location_id, max(created_at) as max_date from warehouse_raw_data group by location_id) as t2, location as l left join warehouse_raw_data w on l.id = w.location_id left join user u on l.user_id = u.id\n' +
